...
class HandTracker:
    ...
    def volume_regulator(self, image, co_ords_list, volume_range=(0, 0), hand_range=(0, 0)):
        thumb_coords = (0, 0)
        index_coords = (0, 0)
        distance = -1
        volume = 0
        ...
        if not(self.left_hand_in_frame(co_ords_list) or self.no_hand_in_frame(co_ords_list)):
            ...
            volume = np.interp(distance, hand_range, volume_range)
            distance = np.interp(distance, hand_range, (0, 100))

        return int(distance), volume
